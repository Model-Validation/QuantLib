...
namespace QuantLib {
    ...
    Real MidPointCdsEngine::survivalProbability(const Date& d) const {
    ...
    Real MidPointCdsEngine::expectedLoss(const Date& defaultDate,
                                         const Date& d1,
                                         const Date& d2,
    ...
    void MidPointCdsEngineBase::calculate(const Date& refDate,
                                          const CreditDefaultSwap::arguments& arguments,
        ...
        if (arguments.accrualRebateCurrent && !arguments.accrualRebateCurrent->hasOccurred(
        ...
        for (Size i = 0; i < arguments.leg.size(); ++i) {
            ...
            ext::shared_ptr<Coupon> coupon =
                ext::dynamic_pointer_cast<Coupon>(arguments.leg[i]);
            QL_REQUIRE(coupon,
                       "MidPointCdsEngine: expected coupon, simple cash flows are not allowed");
            ...
            if (arguments.settlesAccrual) {
                results.couponLegNPV += P * coupon->accruedAmount(defaultDate) *
            ...
            Real expectLoss =
                expectedLoss(defaultDate, effectiveStartDate, endDate, coupon->nominal());
            results.defaultLegNPV += expectLoss * midpointDiscount;
        ...
        if (results.couponLegNPV != 0.0) {
            results.fairSpreadDirty = -results.defaultLegNPV * arguments.spread /
                                      (results.couponLegNPV + results.accrualRebateNPV);
            results.fairSpreadClean = -results.defaultLegNPV * arguments.spread /
                                      (results.couponLegNPV + results.accrualRebateNPVCurrent);
        } else {
        ...
        if (upfrontSensitivity > 0.0) {
            results.fairUpfront =
                -upfrontSign *
                (results.defaultLegNPV + results.couponLegNPV + results.accrualRebateNPV) /
                upfrontSensitivity;
        } else {
            results.fairUpfront = Null<Rate>();
        }

        static constexpr Rate basisPoint = 1.0e-4;
        ...
        results.additionalResults["upfrontPremium"] =
            arguments.upfrontPayment->amount() * upfrontSign;
        ...
        results.additionalResults["premiumLegNPVClean"] =
            results.couponLegNPV + results.accrualRebateNPVCurrent;
