...
namespace QuantLib {
    ...
    VannaVolgaPartialTimeBarrierEngine::VannaVolgaPartialTimeBarrierEngine(
    ...
    void VannaVolgaPartialTimeBarrierEngine::calculate() const {
        ...
        Handle<Quote> atmVolQuote(ext::make_shared<SimpleQuote>(atmVol_->value())); // used for
                                                                                    // shift
        ...
        ext::shared_ptr<BlackScholesMertonProcess> stochProcess =
            ext::make_shared<BlackScholesMertonProcess>(x0Quote, foreignTS_, domesticTS_,
                                                        Handle<BlackVolTermStructure>(blackVolTS));

        ext::shared_ptr<PricingEngine> engineBS =
            ext::make_shared<AnalyticPartialTimeBarrierOptionEngine>(stochProcess);

        BlackDeltaCalculator blackDeltaCalculatorAtm(
            Option::Call, atmVol_->deltaType(), x0Quote->value(), domesticTS_->discount(T_),
            foreignTS_->discount(T_), atmVol_->value() * sqrt(T_));
        ...
        BlackDeltaCalculator blackDeltaCalculatorPut25(
            Option::Put, vol25Put_->deltaType(), x0Quote->value(), domesticTS_->discount(T_),
            foreignTS_->discount(T_), put25Vol * sqrt(T_));
        Real put25Strike = blackDeltaCalculatorPut25.strikeFromDelta(-0.25);
        BlackDeltaCalculator blackDeltaCalculatorCall25(
            Option::Call, vol25Call_->deltaType(), x0Quote->value(), domesticTS_->discount(T_),
            foreignTS_->discount(T_), call25Vol * sqrt(T_));
        ...
        VannaVolga vannaVolga(x0Quote->value(), domesticTS_->discount(T_), foreignTS_->discount(T_),
                              T_);
        ...
        Real vanillaOption = blackFormula(payoff->optionType(), payoff->strike(), forward,
        ...
        else if (x0Quote->value() >= arguments_.barrier &&
        ...
        // spot < barrier down&in vanilla
            ...
            PartialTimeBarrierOption partialTimeBarrierOption(
                barrierType, arguments_.barrierRange, arguments_.barrier, arguments_.rebate,
                arguments_.coverEventDate,
                ext::dynamic_pointer_cast<StrikedTypePayoff>(arguments_.payoff),
                arguments_.exercise);

            partialTimeBarrierOption.setPricingEngine(engineBS);

            // BS price with atm vol
            Real priceBS = partialTimeBarrierOption.NPV();

            Real priceAtmCallBS =
                blackFormula(Option::Call, atmStrike, forward, atmVol_->value() * sqrt(T_),
                             domesticTS_->discount(T_));
            Real price25CallBS =
                blackFormula(Option::Call, call25Strike, forward, atmVol_->value() * sqrt(T_),
                             domesticTS_->discount(T_));
            Real price25PutBS =
                blackFormula(Option::Put, put25Strike, forward, atmVol_->value() * sqrt(T_),
                             domesticTS_->discount(T_));

            // market price
            Real priceAtmCallMkt =
                blackFormula(Option::Call, atmStrike, forward, atmVol_->value() * sqrt(T_),
                             domesticTS_->discount(T_));

            Real price25CallMkt = blackFormula(Option::Call, call25Strike, forward,
                                               call25Vol * sqrt(T_), domesticTS_->discount(T_));
            Real price25PutMkt = blackFormula(Option::Put, put25Strike, forward,
            ...
            Real d1atm =
                (std::log(forward / atmStrike) + 0.5 * std::pow(atmVolQuote->value(), 2.0) * T_) /
            ...
            Real vanna25Call_Analytical = vega25Call_Analytical / x0Quote->value() *
            ...
            Real d125Put =
                (std::log(forward / put25Strike) + 0.5 * std::pow(atmVolQuote->value(), 2.0) * T_) /
                (atmVolQuote->value() * sqrt(T_));
            Real vega25Put_Analytical =
                x0Quote->value() * norm(d125Put) * sqrt(T_) * foreignTS_->discount(T_);
            Real vanna25Put_Analytical = vega25Put_Analytical / x0Quote->value() *
            ...
            partialTimeBarrierOption.recalculate();
            Real vegaBarBS = (partialTimeBarrierOption.NPV() - priceBS) / sigmaShift_vega;
            ...
            partialTimeBarrierOption.recalculate();
            Real priceBS2 = partialTimeBarrierOption.NPV();

            // shifted npv
            ext::static_pointer_cast<SimpleQuote>(atmVolQuote.currentLink())
                ->setValue(atmVolQuote->value() + sigmaShift_vega);
            partialTimeBarrierOption.recalculate();
            Real vegaBarBS2 = (partialTimeBarrierOption.NPV() - priceBS2) / sigmaShift_vega;
            ...
            partialTimeBarrierOption.recalculate();
            Real priceBS_delta1 = partialTimeBarrierOption.NPV();

            ext::static_pointer_cast<SimpleQuote>(x0Quote.currentLink())
                ->setValue(x0Quote->value() - 2 * spotShift_delta); // shift back
            partialTimeBarrierOption.recalculate();
            Real priceBS_delta2 = partialTimeBarrierOption.NPV();
            ...
            ext::static_pointer_cast<SimpleQuote>(x0Quote.currentLink())
                ->setValue(x0Quote->value() + spotShift_delta); // shift forth
            partialTimeBarrierOption.recalculate();
            priceBS_delta1 = partialTimeBarrierOption.NPV();

            ext::static_pointer_cast<SimpleQuote>(x0Quote.currentLink())
                ->setValue(x0Quote->value() - 2 * spotShift_delta); // shift back
            partialTimeBarrierOption.recalculate();
            priceBS_delta2 = partialTimeBarrierOption.NPV();
            ...
            Real mu = domesticTS_->zeroRate(T_, Continuous).rate() -
                      foreignTS_->zeroRate(T_, Continuous).rate() -
                      pow(atmVol_->value(), 2.0) / 2.0;
            Real h2 = (log(arguments_.barrier / x0Quote->value()) + mu * T_) /
                      (atmVol_->value() * sqrt(T_));
            Real h2Prime = (log(x0Quote->value() / arguments_.barrier) + mu * T_) /
                           (atmVol_->value() * sqrt(T_));
            Real probTouch = 0.0;
