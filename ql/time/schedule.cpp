...
namespace QuantLib {
    ...
    Schedule::Schedule(Date effectiveDate,
        ...
        if (firstDate_ != Date()) {
            switch (*rule_) {
                ...
                default:
                    QL_FAIL("unknown rule (" << static_cast<Integer>(*rule_) << ")");
            }
        }
            ...
            case DateGeneration::Forward:
                ...
                if (firstDate_ != Date()) {
                    ...
                    if (*rule_ == DateGeneration::OldCDS) {
                        ...
                        static constexpr Date::serial_type stubDays = 30;
                    ...
                    if (next20th != effectiveDate) {
                        ...
                        isRegular_.push_back(
                            *rule_ == DateGeneration::CDS || *rule_ == DateGeneration::CDS2015);
                        seed = next20th;
                    }
                }
    ...
    MakeSchedule::operator Schedule() const {
        ...
        return {effectiveDate_, terminationDate_, *tenor_,
                calendar, convention, terminationDateConvention,
                rule_, endOfMonth_, firstDate_,
                nextToLastDate_};
    }
    ...
    Date cdsMaturity(const Date& tradeDate, const Period& tenor, DateGeneration::Rule rule) {
        ...
        QL_REQUIRE(
            rule == DateGeneration::CDS2015 || rule == DateGeneration::CDS ||
            rule == DateGeneration::OldCDS,
        ...
        QL_REQUIRE(maturity > tradeDate, "error calculating CDS maturity. Tenor is "
                   << tenor << ", trade date is "
                   << io::iso_date(tradeDate)
                   << " generating a maturity of "
                   << io::iso_date(maturity) << " <= trade date.");
