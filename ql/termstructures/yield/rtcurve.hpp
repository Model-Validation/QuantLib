...
/*! \file rtcurve.hpp
    \brief RT-based interpolated curve
*/
...
namespace QuantLib {
    ...
    Rate InterpolatedRateTimeCurve<T>::zeroYieldImpl(Time t) const {
        if (t == 0.0)
            return static_cast<Rate>(this->interpolation_.derivative(0.0, true));

        if (t <= this->times_.back())
            return static_cast<Rate>(this->interpolation_(t, true) / t);

        // flat r extrapolation
        return static_cast<Rate>(this->data_.back() / this->times_.back());
    }
