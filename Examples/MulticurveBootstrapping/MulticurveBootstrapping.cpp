...
int main(int, char* []) {
    ...
    try {
        ...
        *********************/
        ...
        std::vector<ext::shared_ptr<RateHelper> > siorInstruments;

        ext::shared_ptr<SEKSior> sior(new SEKSior);
        ...
        std::map<Natural, ext::shared_ptr<Quote>> depoQuotes = {
            ...
            {1, ext::make_shared<SimpleQuote>(0.00184)},
        };
        ...
        for (const auto& q : depoQuotes) {
            ...
            siorInstruments.push_back(helper);
        }

        // short-term OIS

        std::map<Period, ext::shared_ptr<Quote>> shortOisQuotes = {
            {1 * Weeks, ext::make_shared<SimpleQuote>(0.0051)},
            {2 * Weeks, ext::make_shared<SimpleQuote>(0.00585)},
            {1 * Months, ext::make_shared<SimpleQuote>(0.00625)},
            {2 * Months, ext::make_shared<SimpleQuote>(0.00645)},
            {3 * Months, ext::make_shared<SimpleQuote>(0.00755)},
            {4 * Months, ext::make_shared<SimpleQuote>(0.00915)},
            {5 * Months, ext::make_shared<SimpleQuote>(0.01035)},
            {6 * Months, ext::make_shared<SimpleQuote>(0.01195)},
            {7 * Months, ext::make_shared<SimpleQuote>(0.0133)},
            {8 * Months, ext::make_shared<SimpleQuote>(0.01455)},
            {9 * Months, ext::make_shared<SimpleQuote>(0.0157)},
            {10 * Months, ext::make_shared<SimpleQuote>(0.01665)},
            {11 * Months, ext::make_shared<SimpleQuote>(0.0177)},
            {12 * Months, ext::make_shared<SimpleQuote>(0.0185)},
            {2 * Years, ext::make_shared<SimpleQuote>(0.0240473578)},
            {3 * Years, ext::make_shared<SimpleQuote>(0.0253998865)},
            {4 * Years, ext::make_shared<SimpleQuote>(0.0254598811)},
            {5 * Years, ext::make_shared<SimpleQuote>(0.0254997802)},
            {6 * Years, ext::make_shared<SimpleQuote>(0.025713)},
            {7 * Years, ext::make_shared<SimpleQuote>(0.025875)},
            {8 * Years, ext::make_shared<SimpleQuote>(0.026063)},
            {9 * Years, ext::make_shared<SimpleQuote>(0.026225)},
            {10 * Years, ext::make_shared<SimpleQuote>(0.0263248973)},
            {12 * Years, ext::make_shared<SimpleQuote>(0.02635)},
            {15 * Years, ext::make_shared<SimpleQuote>(0.0263498717)},
            {20 * Years, ext::make_shared<SimpleQuote>(0.025825)},
            {25 * Years, ext::make_shared<SimpleQuote>(0.025025)},
            {30 * Years, ext::make_shared<SimpleQuote>(0.024125)}
        };
        ...
        for (const auto& q : shortOisQuotes) {
            ...
            auto helper = ext::make_shared<OISRateHelper>(
                2, tenor, Handle<Quote>(quote), sior,
        ...
        ext::shared_ptr<PiecewiseYieldCurve<ZeroYield, SplineCubic>> siorTermStructure(
            new PiecewiseYieldCurve<ZeroYield, SplineCubic>(
                todaysDate, siorInstruments,
                termStructureDayCounter) );

        siorTermStructure->enableExtrapolation();

        // This curve will be used for discounting cash flows
        RelinkableHandle<YieldTermStructure> discountingTermStructure;
        discountingTermStructure.linkTo(siorTermStructure);
        ...
        *********************/
        ...
        for (auto pair : siorTermStructure->nodes()) {
            std::cout << io::iso_date(pair.first);
            std::cout << "\t" << std::setprecision(7) << pair.second;
            std::cout << std::endl;
        }

        for (Date date : siorTermStructure->dates()) {
            std::cout << io::iso_date(date);
            std::cout << "\t" << std::setprecision(7) << siorTermStructure->zeroRate(date, Actual360(), Continuous, NoFrequency, false);
            std::cout << std::endl;
        }
